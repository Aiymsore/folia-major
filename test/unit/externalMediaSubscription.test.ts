...
// test/unit/externalMediaSubscription.test.ts
...
//
// 最后两条是 production lifecycle 契约，不是测试技巧：`externalMediaGetState()` 是一次 IPC，
// 慢 IPC 下迟到的解析会把过期快照盖在更新鲜的状态上。
//
// 清理一律走 `stopExternalMediaSubscription()`。绝不能用一个「start 再 cleanup」来充当 reset：
...
/** 每个测试可以决定 externalMediaGetState 返回什么；默认立即成功。 */
let getStateImpl: () => Promise<ElectronExternalMediaStatus>;
...
describe('Apple Music SMTC subscription lifecycle', () => {
    ...
    it('subscribes exactly once across repeated starts and applies pushed snapshots', () => {
        ...
        expect(useExternalMediaStore.getState().status?.title).toBe('Pushed');

        handlers[0](makeStatus({ title: 'Pushed Again' }));
        expect(useExternalMediaStore.getState().status?.title).toBe('Pushed Again');
    });
    ...
    it('cleans up exactly once and can be restarted afterwards', async () => {
        ...
        const restarted = startExternalMediaSubscription();
        expect(handlerAdds).toBe(2);
        expect(handlers).toHaveLength(1);

        await Promise.resolve();
        handlers[0](makeStatus({ title: 'After Restart' }));
        expect(useExternalMediaStore.getState().status?.title).toBe('After Restart');
    ...
    it('does not clear the playback snapshot when the subscription stops', () => {
        ...
        expect(useExternalMediaStore.getState().status?.title).toBe('Still Playing');
    ...
    it('keeps a pushed snapshot authoritative over the initial read', async () => {
        let resolveInitial: (status: ElectronExternalMediaStatus) => void = () => {};
        ...
        expect(useExternalMediaStore.getState().status?.title).toBe('Pushed First');
    });
    ...
    it('ignores an initial read that resolves after cleanup', async () => {
        ...
        expect(useExternalMediaStore.getState().status).toBeNull();
    });

    it('never lets a previous generation overwrite the current one', async () => {
        let resolveA: (status: ElectronExternalMediaStatus) => void = () => {};
        ...
        startExternalMediaSubscription();
        await flushMicrotasks();
        expect(useExternalMediaStore.getState().status?.title).toBe('Generation B');
