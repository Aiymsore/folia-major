...
export default {
  ...
  "ponder": {
    ...
    "summaries": {
      "audio_equalizer": "十段均衡加一条效果链 —— 而拖一下就会覆写一个自定义槽。",
      "vis_playground": "预览上压着三块看不见的点击热区。",
      "theme_park": "整屏配色编辑器：在编辑哪一份、哪一面，以及保存为什么按不动。",
      "custom_shortcut_settings": "Alt 加一颗字母，以及这份命令列表为什么比命令窗口里的短。",
      "pinned_commands": "命令窗口下面那三颗，和「最近用过的排前面」不是一回事。",
      "replay_gain_settings": "和来源页上那个三选一是同一个值。",
      "import_export_settings": "它带走的是外观，不是你的曲库 —— 而且导入会先问过你。",
    ...
    "captions": {
      "audioEqualizer": {
        "enable": "音量行右端那颗推子图标打开的就是这扇对话框。左上那颗是整条链的总开关；关掉之后下面所有的值都还留着，只是不再作用到声音上。",
        ...
        "silent": "下面这件事屏幕上一个字都没写：停在内置预设上时，拖动**任何一根**推子，都会把你静默切到「自定义 1」并把值写进去。没有提示，也没有确认 —— 而自定义 1 里原来存的东西就这么没了。如果自定义 1 里存着你调好的一套，动内置预设之前先切到自定义 2。",
        "reset": "排尾那颗圆按钮清空的是**当前**那个自定义槽，把它退回平直曲线加一套中性效果。它只动这一个槽；停在内置预设上时它是灰的 —— 一个不能编辑的东西没有什么可退回的。",
        "effects": "推子排下面是这条链的其余部分：高低切、饱和、位压缩、抖晃、噪声、立体声宽度、空间和冲击。它们写进的是同一个自定义槽，走的是同一条静默规则。",
        "noise": "其中几项标题旁边那枚小徽章，意思是这个效果自己会带进噪声。不管推子推没推上去它都显示着 —— 为的是让「这嘶声哪来的」在你打开它之前就有答案，而不是之后。"
      },
      "visPlayground": {
        "preview": "调参台是整屏打开的：左边一块实时预览，右边一条窄设置栏。左边那块是真的在跑 —— 动画、背景和字幕用的都是真正的渲染器，不是一张静态图，所以改什么立刻就看得见。",
        "pause": "预览右下角那颗按钮把画面冻在当前这一帧。调那种只闪一下的效果时用得上，顺带也让预览在你读字的时候不再继续吃算力。",
        "invisible": "这块预览上有样东西大多数人一辈子都发现不了：压着三块点击热区。它们没有描边、没有底色、没有标签 —— 完全没有任何东西标出它们。",
        ...
        "fonts": "「通用」里大半是歌词的字：字体、字号、字重 —— 字重默认是自动的，除非你手动接管，因为合适的默认值取决于你刚挑的那个字体。",
        ...
        "perMode": "模式下面跟着的是这个模式自己的调参。它不是一组固定的行：换个模式它们就跟着换 —— 所以你记得的某根推子会不见，那是因为它属于你刚离开的那一个。",
        "background": "背景那一页是同一套做法：先挑背景，再调这个背景。它和动画是两层，各挑各的，任意组合都成立。",
        "subtitleContent": "字幕是歌词下面那第二行，第一行决定它里面放什么：什么都不放、翻译，或者罗马音。这是一个三选一，不是三个开关 —— 翻译和罗马音不能同时开着。",
        "subtitleLegibility": "下面几行管的是「在一张动着的画面上还读不读得清」：文字后面垫一层底、这层底多不透明，以及给还没唱到的那几行加一点模糊，让当前这行浮出来。",
        "subtitleFont": "最后一行是字幕自己的字，它默认跟着歌词的字走。关掉它，字幕自己的字体、字号、字重才会在下面长出来 —— 开着的时候它们根本不在那儿，这就是「字幕字体设置怎么找不到」的答案。"
      },
      "themePark": {
        "which": "右上角那一对切的是这次编辑的是哪一份主题：当前这首歌的 AI 主题，还是你保存的自定义主题。两份草稿在编辑器开着期间同时活着，来回切不会丢东西 —— 但保存按钮写的永远只是此刻选中的那一份，按钮上的字也跟着它变。",
        "reset": "重置丢掉的是你这一次改的东西，把草稿退回刚打开时的样子。它不碰已经保存下来的那一份。",
        "twoSides": "亮和暗是两份独立的配色，不是一份配色加一个预览开关。这一对决定的是你此刻在编辑哪一面，它下面的一切 —— 四种颜色、取色器、HEX 框 —— 都跟着这个选择走。",
        "fourColors": "每一面四种颜色：背景、主色、强调色、次要色。点某一行就把它送进下面的取色器；每个名字下面那行小字写的是这个颜色实际落在应用的哪儿。",
        ...
        "nameLivesHere": "而填名字的地方不在这一页。它和描述一起在「信息」那一页上 —— 按不动的那颗按钮和能解决它的那个输入框，分在两个标签里。保存按不动的时候，先去那一页。",
        "applies": "关于保存还有一件事：存进自定义主题的同时，应用也会切换到它。它不是悄悄写进一个槽、等你以后再去挑 —— 你刚配出来的这套颜色，就是接下来你要一直看着的那套。"
      },
      "customShortcut": {
        "alt": "这一行左半边是两颗键帽，只有第二颗是你的。Alt 画成已经按下去的样子，因为它改不了：这条快捷键永远是 Alt 加一颗字母，想在这儿录 Ctrl+Shift+X 那样的组合是录不进去的。",
        "capture": "点第二颗键帽它就开始收键，下一次按下的那颗就是绑定的字母。过程中按住的修饰键会被忽略，按 Esc 直接退出、什么都不录。",
        "taken": "按到一颗已经被占掉的字母会当场退回，红字写在键帽底下说明原因。S 和 Ctrl+K 是命令窗口自己的，注册表里每条带热键的命令也各占一颗 —— 所以这份占用名单会随着 Folia 增加快捷键自己变长。",
        "clear": "录上之后键帽旁边会多一颗小小的 ✕，按它就把绑定清掉；在你重新录一颗之前，这条快捷键什么都不运行。",
        "filtered": "右边那个下拉并不列出全部命令。快捷键在哪儿都能按，所以只有在哪儿都成立的命令才进得来 —— 凡是要求停在某一页、某块面板上的都被筛掉了。于是你在命令窗口里找得到的某条命令，这里可能根本没有，而屏幕上不会解释这个差。",
        "goesQuiet": "同一道判断在按下去的时候还会再跑一遍，不只是挑的时候。某条命令后来多了前提条件，或者你那颗字母被注册表占走了，这条绑定就会安静下来、而不是去做别的事 —— 所以一条悄悄失灵的快捷键，值得回到这里重挑一次。"
      },
      "pinnedCommands": {
        "slots": "三个槽位，各是一个下拉。放进去的命令会按这个顺序出现在命令窗口下面 —— 槽位 1 在最左。默认放的是上一首、下一首和队列面板。",
        "unique": "一个命令只能占一个槽：选中之后，另外两个下拉里就不再列它。某个槽留空，它的位置就空着；三个都留空，那一排在窗口里整个不出现。",
        "where": "这三个下拉配出来的东西长在这儿：命令窗口下面一排三颗固定按钮，位置永远不变。点一下就执行，不用打字也不用翻。",
        "recent": "它们上面那份列表是另一套机制，而且是完全独立的一套：它会自己重排，最近运行过的浮到前面，常用的再往上顶一点。固定的那三颗不参与这件事 —— 它们存在另一个地方，永远不动。「我明明固定了，位置怎么还在变」就是把这两样看成了一样。",
        "empty": "所以两者各管各的用处：列表应付你临时要用的东西，那三颗应付你希望每次都在同一个位置的东西。"
      },
      "replayGain": {
        "off": "ReplayGain 是写在文件自己里的一段响度标签。关掉它，Folia 就不理这段标签，每首歌按母带的电平原样播 —— 这也是为什么一张专辑会比下一张响出一截。",
        "trackAlbum": "「按单曲」是拿每首歌和所有别的歌找齐，随机播一整个队列时音量最稳。「按专辑」是整张碟用同一个数，专辑里该轻的仍然轻、该响的仍然响，保留混音时的动态。两种都不会重新编码文件，动的只是播放音量。",
        "sameValue": "控制面板的来源页上有同样这三颗按钮，它们不是第二个设置：两处读写的是同一个值，在任何一边改，两边都跟着变。而这件事在任何一边都没有写出来。",
        "summary": "来源页比这里多的是那一行：这首歌自己的标签，写成 T 和 A 两个分贝数，文件里没带就显示「不可用」。模式选了却听不出区别时，答案通常就在那一行 —— 没有标签，也就没有东西可以套用。"
      },
      "importExport": {
        "scope": "这一组带走的是 Folia 长什么样：配色主题、歌词动画和各个渲染器的调参、字幕与字体、背景，以及曲目卡片。全部就这些。",
        "notBackup": "所以名字虽然叫备份，它并不是 Folia 的备份。歌单、本地曲库、播放设置、快捷键、账号全都在它之外 —— 出了事拿这串东西去恢复、指望把音乐找回来，正是这个名字最容易引出的误会。",
        "themeChoice": "导出前先挑带哪个主题：AI 生成的那套、你自己调的那套，或者一个都不带。选「不带」就只走视觉设置、把你的配色留下，想分享一套歌词动画而不是一套配色时该用它。",
        "clipboard": "两颗按钮都是复制，不是存文件：一颗给你一串短码，另一颗给同样的东西、但是可读的 JSON。贴到哪儿都行 —— 聊天、笔记 —— 那就是整份配置。",
        "paste": "导入走的是同一个框的反方向：把别人那串码粘进来，右边那颗导入就亮了。",
        "plan": "导入不会照单全收。它先算出这份配置会改动什么，按组摊给你看，每一行左边是你现在的值、右边是即将变成的值 —— 而且每行各有一个勾选框，只要背景不要字体是完全正常的做法。",
        "derived": "最底下那一截才是这个确认框存在的理由：配置里根本没提、却会跟着一起发生的改动。接受别人的配色，应用就会切到自定义主题；接受一个系统字体，你上传的那个字体文件会被删掉。它们没法单独取消勾选 —— 要挡的话，去取消引起它们的那一条。"
      },
      "onboarding": {
        ...
        "touchBulb": "触屏没有悬停这回事，所以那条路整个走不通。取而代之的是右上角这颗灯泡，它只在触屏设备上出现 —— 用鼠标的话找不到它是正常的。它落地或换页时露几秒就自己收起来，免得挡住界面；想再叫出来，点一下右上角那块地方。",
      ...
      "pages": {
        ...
        "latticeChrome": "展开海报底部包含播放/暂停、额外槽位、时间、进度拖动和“回到 Player”。触屏再点展开海报、键盘按 Space 可显隐控制条。",
      ...
      "playerBar": {
        "basicsAutoExpand": "展开它的不只有悬停。只要播放是暂停的、而且你不在首页，这条胶囊就自己一直展开着 —— 最可能想伸手的那一刻，它正好不再把控件收起来。重新播放，或者回到首页，它就又瘦回去了。",
      ...
      "sidePanel": {
        ...
        "controlsModeList": "中间那块名称是一颗按钮，而屏幕上没有任何地方说它是。点一下，完整列表就压下来，可以直接跳到想要的那一个，不必一格一格地从中间全部走过去。",
        "controlsModeListFooter": "列表最底下那一条不是第七个模式 —— 它打开的是这一行对应的完整设置，调参就在那儿。",
        "queueRadio": "私人 FM 放着的时候，这一格不是队列。同一格标签整格换成电台面板 —— 图标变了、名字变了，而「接下来是什么」那份清单不在了，因为电台本来就没有队列。",
        "queueRadioMode": "顶上那枚胶囊写的是你现在在哪个电台模式；点它打开的是命令窗口里的模式选择器，和命令面板给的是同一个。",
        "queueRadioActions": "最底下那一对是扔掉和喜欢。扔掉不是「把这一行移掉」—— 这里根本没有行：它是告诉服务别再给你放这首，然后电台直接往下走。",
        "cover": "面板最上面是当前歌曲的封面，占满面板宽度的一整块正方形。它不属于任何一个标签页 —— 换页时它留在原处。指针移上去，四个角会各浮出一颗按钮。",
        "tabs": "封面下面紧挨着的这一排就是标签页：封面、控制、队列、账号。当前这首来自本地、Navidrome 或在线时，封面页后面还会插进一格来源页。它们是同一块地方的几副面孔，换页不会挪走面板。",
        "body": "标签排下面这一整块是当前标签页的内容。面板的结构就这两段 —— 一张封面，加上一排标签和它下面的内容，中间没有第三段常驻信息。",
        "cycle": "不必去点那一排小格子。面板开着时按 Tab，就在标签页之间往后循环一格。",
        "cycleReverse": "Shift + Tab 往回一格。循环只在当前真的有的那几页之间走 —— 来源不同，页数也不同。",
        "sourceTab": "这一格不是固定的。当前这首来自本地文件、Navidrome 或在线来源时，它才插在封面页后面，图标也跟着来源变；换一首没有来源信息的歌，标签排就只剩四格。",
        "sourceInfo": "最上面是这首歌在它来源那一侧的信息：本地文件给文件名、大小、码率和歌词状态，Navidrome 给服务器上的 Song ID，在线来源没有这一块。",
        "sourceGain": "「音频增益」就是 ReplayGain，关闭 / 单曲 / 专辑三选一。右上角写的是这首歌自带的增益值，T 是单曲、A 是专辑；文件里没有就是不可用。",
        "sourceLyrics": "歌词那一行右端两颗小图标：一颗导入本地歌词文件（lrc / vtt / ttml / qrc / yrc / krc / txt），一颗去在线匹配。下面那条写着当前用的是哪一份；导入过之后会多出一格可以切回去。",
        "sourceOffset": "最底下的「时间轴偏移」按 250ms 一档左右调，也可以直接输数字，正数让歌词更晚出现。它只对本次播放临时生效；要长期改，设置里另有一个全局偏移，两者相加。",
        "coverTab": "封面页放的是当前这首歌的文字信息：歌名、歌手、专辑，居中排一列。大封面不在这一页里，它常驻在面板顶上。",
        "coverTabDetail": "歌手和专辑都能点，点进去是对应的集合。歌名点一下是把「歌名 - 歌手 - 专辑」连同来源页链接一起复制走；按住 Ctrl 再点，直接打开这首歌在来源平台的页面。",
        "controlsTab": "控制页最上面是三颗大按钮：循环模式、喜欢、生成主题。这是整块面板里唯一一排大触控目标。",
        "controlsTabDetail": "下面才是边听边调的那些：一行音量，右端带均衡器和自动混音；两行「‹ 模式 ›」取景器分别换歌词动画和背景；最底下是配色来源和当前主题的快速编辑。音量增益和歌词时间轴偏移在来源那一格里，不在这页。",
      ...
      "panelCoverActions": {
        ...
        "transparent": "右上角那颗是「播放页透明背景」，四颗里最容易按错的一颗：它把整个窗口切成透明模式，给 OBS 浏览器源或者抠像叠加用，只对播放页生效。",
      ...
      "lyricsAnimation": {
        ...
        "playgroundSections": "右栏顶上分「通用 / 背景 / 动画 / 字幕」四页。歌词动画模式在「动画」那一页里挑，选中哪个播放页就用哪个；那一页下面跟着的是当前这个模式自己的参数。",
        "card": "入口下面这一张卡里装着两个开关。它们是同一张卡的上下两行，中间只隔一条分隔线，不是两张并排的卡。",
        "transparent": "上面那行「播放页透明背景」只对播放页生效：开了之后窗口切成透明模式，适合 OBS 浏览器源或者抠像叠加。它和封面右上角那颗按钮是同一个开关。",
      ...
      "themeSettings": {
        ...
        "source": "自定义配色不是手调出来的 —— 这里选它从哪来，只有两个选项：跟着当前封面取色，或者交给 AI 读歌词推断。没配好服务商 API Key 时，AI 那个是禁用的。",
        "themePark": "标题右边那颗圆形的调色板按钮打开 Theme Park。它不是一个配色库 —— 是整屏的配色编辑器：左边拿真实的可视化实时预览，改一个颜色立刻看得到。",
        "themeParkTabs": "右边那条栏分「配色 / 信息 / 内容 / AI」四页：配色页逐项挑颜色，亮色和暗色各存一套；AI 页让模型直接生成一整套。存下来的就是上面那张「自定义」。",
        ...
        "autoSwitch": "「主题自动切换」相反：切到一首曾经生成过主题的歌时，自动把那套配色应用上。打开它，下面还会多出一行「自动为播放歌曲进行主题生成」，让没有缓存主题的歌也现场生成一套。"
      },
      "panelSlide": {
