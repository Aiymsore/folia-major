...
import { hasExternalMedia, resolveExternalMediaAvailability } from '../utils/externalMediaStatus';

// src/stores/useExternalMediaStore.ts
// 外部媒体快照的唯一状态源（只读）。
//
// 为什么统一成 store，而不是「一个 hook + 一个 .getState()」：
//   * 订阅 main 进程广播的动作只能发生一次。做成 hook 的话，每个调用点都会各自
//     `onExternalMediaStateChanged` 一次，得到彼此独立的 snapshot，并且谁都可能落后一帧。
//   * 非渲染路径（命令派发、能力判断）必须能同步直读，所以需要 `getExternalMediaStatus()`。
//   两者读的是同一份对象，因此不存在「hook 版」和「store 版」两个真相。
//
// 本 store 是纯只读的投影：它不写 currentSong / audioSrc / queue / audioRef，
// 也不修改 activePlaybackBackend（外部媒体状态变化绝不切换后端）。
//
// 判据本身（是否算「有内容」、六态如何映射）在 utils/externalMediaStatus.ts，依赖无关、可单测。

export { hasExternalMedia };

type ExternalMediaStore = {
    status: ElectronExternalMediaStatus | null;
    setStatus: (next: ElectronExternalMediaStatus) => void;
};

export const useExternalMediaStore = create<ExternalMediaStore>(set => ({
    status: null,
    setStatus: next => set({ status: next }),
}));

/** 非渲染路径的直读入口。 */
export const getExternalMediaStatus = (): ElectronExternalMediaStatus | null => (
    useExternalMediaStore.getState().status
);

export const getExternalMediaAvailability = (): ExternalMediaAvailability => (
    resolveExternalMediaAvailability(getExternalMediaStatus())
);

/** 是否有可播放的曲目（session + 曲目信息），与播放状态无关。 */
export const hasExternalMediaSession = (): boolean => hasExternalMedia(getExternalMediaStatus());

/**
 * 扩展通道是否就绪（桥可达 + 扩展已连接）。
 *
 * 与「有曲目」是两件事，刻意分开：`playById` 需要的是扩展，而位置/状态可以只靠 SMTC 观察。
 * 合成一个判据会让「扩展掉线但 SMTC 仍在报告」这种常见状态无法表达 —— 而那正是 MV3
 * service worker 被回收时的常态。
 */
export const isExternalMediaTransportReady = (): boolean => {
    const status = getExternalMediaStatus();
    return Boolean(status?.bridgeAvailable) && Boolean(status?.extensionConnected);
};
 ...
 *
 * 为什么需要它：`externalMediaGetState()` 是一次 IPC，返回时可能这一代订阅早就被 cleanup 掉了。
 ...
 */
export const stopExternalMediaSubscription = (): void => {
    subscriptionCleanup?.();
};

/**
 * 建立**唯一**一份外部媒体订阅并把快照写进 store。重复调用是 no-op，返回同一个清理句柄。
 *
 * 幂等是这里的关键：它会被 App 调用一次，也可能被提前调用（诊断面板、命令派发），
 * 而任何一次重复订阅都会让 store 被两条广播流交替覆盖。因此**只有真正创建订阅的分支**
 * 才递增代次：重复的 start 既不注册 listener，也不推进代次。
 */
export const startExternalMediaSubscription = (): (() => void) => {
    ...
    if (typeof bridge?.onExternalMediaStateChanged !== 'function') {
    ...
    const unsubscribe = bridge.onExternalMediaStateChanged(status => {
    ...
    if (typeof bridge.externalMediaGetState === 'function') {
        void bridge.externalMediaGetState()
            .then(status => {
                ...
                if (cancelled || subscriptionGeneration !== generation) return;
                if (!useExternalMediaStore.getState().status && status) {
                    useExternalMediaStore.getState().setStatus(status);
                }
            })
            .catch(() => {
...
export const selectExternalMediaAvailability = resolveExternalMediaAvailability;
